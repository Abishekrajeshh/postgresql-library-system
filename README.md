...
![Library_project](https://github.com/Abishekrajeshh/postgresql-library-system/blob/main/library.jpg)
...
### 1. Database Setup
![ERD](https://github.com/Abishekrajeshh/postgresql-library-system/blob/main/library_erd.png)
...
```sql
--Checking what all are Damaged
SELECT * FROM return_status WHERE book_quality = 'Damaged';



--just see all damaged returns
SELECT m.member_id, m.member_name, b.book_title, rs.book_quality
...
SELECT 
    ...
    m.member_name,
    b.book_title,
    COUNT(*) AS times_damaged_books_issued
FROM return_status rs
JOIN issued_status ist ON rs.issued_id = ist.issued_id
JOIN members m ON ist.issued_member_id = m.member_id
JOIN books b ON ist.issued_book_isbn = b.isbn
...
ORDER BY times_damaged_books_issued DESC;
```
...
SELECT 
    ...
    -- Total fines ($0.50 per overdue day after 30 days)
    SUM(
        CASE 
            WHEN rs.return_date IS NULL THEN 
                GREATEST(0, (CURRENT_DATE - ist.issued_date) - 30)
            WHEN rs.return_date > ist.issued_date + INTERVAL '30 days' THEN 
